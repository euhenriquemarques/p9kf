...
const Receita: React.FC = () => {
  ...
  const [tabelaLista, setTabelaLista] = useState<iReceitas[]>([
    {
      id: 0,
      categoria: {
        id: 0,
  ...
  const [orderBy, setOrderBy] = useState<keyof iReceitas>("descricao");
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);
  const handleRequestSort = (
    event: React.MouseEvent<unknown>,
    property: keyof iReceitas
  ) => {
  ...
  const handleRowClick = (item: iReceitas) => {
    setFormData({
      ...item,
  ...
  async function buscarDadosTabela() {
    try {
      const response = await axios.get("http://localhost:8080/receita/todas", {
        headers: {
